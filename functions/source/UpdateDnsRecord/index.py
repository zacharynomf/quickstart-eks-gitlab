...
def get_free_replica_number():
    ...
    reservations = response["Reservations"]
    running_replicas = {
        get_replica_number(instance)
        for r in reservations
        for instance in r["Instances"]
    }
    logger.info(f"running replicas: {running_replicas}")
    free_replicas = ALL_REPLICAS.difference(running_replicas)
    logger.info(f"free replicas: {free_replicas}")
...
def update_dns_record(transition, replica_hostname, instance_ip):
    ...
    logger.info("successfully updated dns")
...
def complete_lifecycle_hook(instance_id, event, action_result):
    try:
        ...
        if check_response(response):
            logger.info(f"Lifecycle hook completed correctly: {response}")
        else:
            logger.info(f"Lifecycle hook could not be completed: {response}")
    except:
        logger.exception(f"Lifecycle hook completion could not be executed")
        return None
    ...
    for record in event["Records"]:
        ...
        if transition in [LAUNCHING_TRANSITION, TERMINATING_TRANSITION]:
            ...
            try:
                ...
                logger.info(f"updating DNS for {replica_hostname} = {instance_ip}")
