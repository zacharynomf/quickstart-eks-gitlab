...
. Edit your local hosts file to add the ip address pointed at the host name (swap out 111.111.111.111 with the IP address from above).  Note "gitlab" added to the beginning. Replace **full.hostname.from.parameterstore** with your value.

----
111.111.111.111 gitlab.*full.hostname.from.parameterstore*
----

[WARNING]
====
Over the course of time, the load balancer may retire this IP address - if this happens, repeat the last 4 steps to get an active load balancer IP to update the /etc/hosts file with.
====