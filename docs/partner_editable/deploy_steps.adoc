...
. Use the Region selector in the navigation bar to choose the AWS Region where you want to deploy GitLab.
...
^|http://qs_launch_permalink[Deploy {partner-product-short-name} into an existing EKS cluster on AWS^]
^|http://qs_template_permalink[View template^]
|===
