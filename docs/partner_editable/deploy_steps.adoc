...
** For `Cache mode` production and performance testing setups must select External to ensure Redis is configured using Elasticache rather than placed in the EKS cluster. Redis in the EKS cluster is not currently supported by GitLab Reference Architecture.
