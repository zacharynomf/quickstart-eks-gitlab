...
.Key to compliance tags
[cols="^1,7"]
|===
|Tag|Meaning

|^[AWS-WA]^|Compliant with **AWS W**ell **A**rchitected - including favoring PaaS to externalize scaling management.
...
|^[IAC-CS]^|**I**nfrastructure **a**s **C**ode **C**omplete **S**olution - sets up everything required to make GitLab operational.
|===
...
internet access for resources in the private subnets.^1^ ^[IAC-CS]^
...
** Elasticache for Redis PaaS. [Eliminates 4 Redis related instances](https://docs.gitlab.com/ee/administration/reference_architectures/10k_users.html)) ^[GL-RAR][AWS-WA]^
** S3 Object storage PaaS for all non-git file systems and git-lfs ^[GL-RAR][AWS-WA]^
** Uses AWS ELB for internal load balancer - Load balancing PaaS. [Eliminates 1 load balancing related instances](https://docs.gitlab.com/ee/administration/reference_architectures/10k_users.html) ^[GL-RAR][AWS-WA]^
** AWS ELB Load Balancing PaaS for external access. [Eliminates external load balancer setup](https://docs.gitlab.com/ee/administration/reference_architectures/10k_users.html)) ^[AWS-WA][IAC-CS]^
...
** Amazon CloudWatch Container Insights integration. ^[AWS-WA][IAC-CS]^
** Amazon CloudWatch Agent for Instance compute. ^[AWS-WA][IAC-CS]^
...
cluster as well as the AWS CLI V2 for general AWS management. ^[AWS-WA][IAC-CS]^
** Amazon Route53 Hosted zone for DNS configuration. ^[AWS-WA][IAC-CS]^
** AWS Certificate Manager TLS certificate. ^[AWS-WA][IAC-CS]^
** Amazon Simple Email Service domain for outgoing email messages. ^[AWS-WA][IAC-CS]^
** IaC parameters and outputs stored in AWS Parameter Store. ^[AWS-WA][IAC-CS]^
...
- All GitLab logs are automatically aggregated into CloudWatch logs via CloudWatch Instance Agent and CloudWatch Container Insights. ^[AWS-WA][IAC-CS]^
- EKS workload monitoring via CloudWatch Container Insights. ^[AWS-WA][IAC-CS]^
- GitLab backup is preconfigured. ^[IAC-CS]^
- Prometheus is always deployed with the GitLab instance. ^[IAC-CS]^
- Setting "ConfigureGrafana" also deploys and configures Grafana. ^[AWS-WA][IAC-CS]^
- The container registry is automatically setup at `registry.[subdomain]` ^[AWS-WA][IAC-CS]^

=== Data Security

* Secrets:
** Stores generated secrets in AWS Secrets Manager (e.g. generates a proper initial root password) ^[AWS-WA][IAC-CS]^
* Data in Transit:
** SSL for front end http. ^[AWS-WA][IAC-CS]^
** SSL for GitLab to S3. ^[AWS-WA][IAC-CS]^
** SSL for GitLab to PostgreSQL RDS. ^[AWS-WA][IAC-CS]^
** SSL for GitLab to Redis Elasticache. ^[AWS-WA][IAC-CS]^
** SSL for GitLab to Redis Gitaly. ^[AWS-WA][IAC-CS]^
** SSL for GitLab to Runner. ^[AWS-WA][IAC-CS]^
** SSL Not Yet Available for GitLab to Praefect to Gitaly. ^[AWS-WA][IAC-CS]^
* Data at Rest (AWS Managed Keys):
** S3 Encryption ^[AWS-WA][IAC-CS]^
** PostgreSQL RDS Encryption ^[AWS-WA][IAC-CS]^
** Redis Elasticache Encryption ^[AWS-WA][IAC-CS]^
** Gitaly EBS Encryption (Git File System) ^[AWS-WA][IAC-CS]^
** Praefect Data (PostgreSQL RDS). ^[AWS-WA][IAC-CS]^

=== Security and Internet Egress
The GitLab Kubernetes endpoint can be private, a bastion host must be deployed in order to do direct cluster management (it will be preloaded with AWS, EKS and Kubernetes management utilities and IAM permissions for cluster management).
...
* EBS Volumes on Gitaly Cluster instances.
